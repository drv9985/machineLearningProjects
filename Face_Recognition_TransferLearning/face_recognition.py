...
def index():
    for key in request.form:
        if key.startswith('comment.'):
            ...
            os.chdir("C:\\Users\\DRV\\Desktop\\Capture")
            os.makedirs(str(value))
            dynamic=("C:\\Users\\DRV\\Desktop\\Capture\\"+str(value))
            os.chdir(dynamic)
            sampleNum=0;
            a=0
            while True:
...
#    print("Saved model to disk")
    #load saved model
    json_file = open("G:\\Json file\\model_27_96.json",'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
    loaded_model.load_weights("G:\\Json file\\model_27_96.h5")
